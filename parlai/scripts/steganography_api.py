...
# Return agent index
def create_agent(is_owned: bool):
...
# Return secret index
def post_secret(agent_id: int, secret: bytes):
...
def has_pending_message(agent_id):
    assert(state.agents is not None and len(state.agents) > agent_id) # Unknown agent otherwise
...
def send_stegotext(agent_id: int):
    assert(state.agents is not None and len(state.agents) > agent_id) # Unknown agent otherwise
...
def receive_stegotext(agent_id: int, text: str):
    assert(state.agents is not None and len(state.agents) > agent_id) # Unknown agent otherwise
...
secret_index = post_secret(my_id, b'The password is secret')

stegotexts = []

while(has_pending_message(my_id)):
    stegotexts.append(send_stegotext(my_id))
    print("    Stegotext: ", stegotexts[-1])
